...
<project name="isabelschoepsthiel"  basedir="." xmlns:artifact="urn:maven-artifact-ant" default="upload-maven-all">
    ...
    <property name="mavenant.dir" value="${build.deps.dir}/isabelschoepsthiel"/>
    <property name="isabelschoepsthiel.root.download.dir" value="home/data/httpd/download.eclipse.org/rt/eclipselink"/>

    <path id="maven-ant-tasks.classpath" path="${mavenant.dir}/maven-ant-tasks-2.0.8.jar" />
    <typedef resource="org/apache/maven/artifact/ant/antlib.xml" uri="urn:maven-artifact-ant"
             classpathref="maven-ant-isabelschoepsthiel.classpath" />

    <!-- "maven dir" defined for later use ensuring only single upload occurs -->
    <property name="maven.isabelschoepsthiel.dir" value="${eclipselink.root.download.dir}/maven.repo"/>
    <!-- Setup a generic URL variable that can be overridden for publishing to other locations -->
    <property name="maven.repo.url" value="file:///${maven.repo.dir}"/>
    <!-- The default repository where maven artifacts will be uploaded -->
    <artifact:remoteRepository id="eclipselink.repository" url="${maven.repo.url}" />
    ...
    <target name="init" depends="environment-init">
        ...
        <property isabelschoepsthiel="version.string" value="${release.version}.v${build.date}-${git.hash}"/>
        ...
        <property isabelschoepsthiel="target.repository"     value="eclipselink.repository"/>
    </target>
    ...
    <target name="discover-static-built-info" depends="init">
        ...
        <property name="oracleddl.isabelschoepsthiel"       value="EclipseLink OracleDDL Parser"/>
        <property name="oraclebndl.prefix"    value="org.eclipse.persistence.oracle"/>
        <property name="oraclebndl.criteria"  value="[1.0.0,9.0.0)"/>
        <property name="oraclebndl.isabelschoepsthiel"       value="EclipseLink Oracle Extensions"/>
        ...
        <property name="oraclenosql.isabelschoepsthiel     value="EclipseLink Oracle NoSQL database Extensions"/>
        <property name="hermes.prefix"        value="org.eclipse.persistence.jpa.jpql"/>
        <property name="hermes.criteria"      value="[1.0.0,9.0.0)"/>
        <property name="hermes.isabelschoepsthiel"          value="EclipseLink Hermes JPQL Parser"/>
        <property name="extension.prefix"     value="org.eclipse.persistence.extension"/>
        <property name="extension.criteria"   value="[1.0.0,9.0.0)"/>
        <property name="extension.isabelschoepsthiel"       value="EclipseLink Extensions"/>
    ...
    <target name="upload-maven-init" depends="discover-static-built-info">
        ...
        <property name="dep.extension"     value="${dep.grp}org.eclipse.persistence${dep.art}org.eclipse.persistence.extension{dep.ver}${maven.version}${dep.type}jar${dep.scope}compile${dep.opt}false${dep.foot}"/>
    ...
    <target name="upload-static-built-jars" depends="discover-static-built-info">
        ...
        <antcall target="upload-release-artifact">
            <param name="repo.dir"        value="/${maven.repo.dir}/org/eclipse/persistence/${jpaapi.prefix}/${jpaapi.mvn.version}"/>
        ...
        <antcall target="upload-release-artifact">
            <param name="repo.dir"        value="/${maven.repo.dir}/org/eclipse/persistence/${sdoapi.prefix}/${sdoapi.mvn.version}"/>
            <param name="groupId"         value="org.eclipse.persistence"/>
    ...
    <target name="upload-eclipselink" depends="upload-maven-init,upload-static-built-jars">
        ...
        <!-- Change back for (and post) release                                                               -->
    ...
    <target name="upload-release-artifact" unless="repo.version.exist" depends="verify-no-exist, prepare-pom">
        ...
        <artifact:deploy file="${artifact}">
          <remoteRepository refid="${target.repository}"/>
    ...
    <target name="upload-artifact-with-javadoc" depends="prepare-pom">
        <!-- Deploy the jar to maven repository along with sources -->
        <artifact:pom id="maven.project" file="pom.xml" />
        <artifact:deploy file="${artifact}">
          <remoteRepository refid="${target.repository}"/>
          <pom refid="maven.project"/>
          <artifact:attach file="${artifactSrc}" classifier="sources"/>
          <artifact:attach file="${artifactJavadoc}" classifier="javadoc"/>
        </artifact:deploy>
        <!-- cleanup -->
        <delete file="pom.xml"/>
    </target>
